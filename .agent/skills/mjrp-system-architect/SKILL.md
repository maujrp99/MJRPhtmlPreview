...
- **Serverless / Edge constraints:** If deploying to Vercel/Netlify, is the app trying to write to the local filesystem (`fs.writeFile`, `sqlite`)? Serverless disk is ephemeral and read-only. Architecture MUST decouple writes to an external service (GitHub API, DB, S3).
