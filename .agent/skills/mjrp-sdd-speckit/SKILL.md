...
**Related Workflows**: `mjrp-sdd`, `mjrp-sdd-specify`, `mjrp-sdd-plan`, `mjrp-sdd-task`, `mjrp-adr`
...
**Cross-check obrigatório (antes de marcar APPROVED)**:
- Cada `US-XXX` de story.md tem um `REQ-XXX` correspondente aqui?
- Cada AC de cada US está refletido em `## Success Criteria`?
- Se ❌ → spec.md está incompleta. Não pode avançar para Plan.
...
**Cross-check obrigatório (antes de marcar APPROVED)**:
- Cada `REQ-XXX` de spec.md tem componente/seção correspondente no plan?
- Cada `US-XXX` de story.md tem pelo menos um componente ou fluxo que a endereça?
- Constraints de spec.md (perf, segurança, a11y) estão no plan — não foram suavizados?
- Se ❌ → plan.md está incompleto. Não pode avançar para Tasking.
...
- Se ❌ → tasks.md está incompleto. Não pode avançar para Implementação.
...
3. **Constitution Alignment**: Plan must not propose dependencies or patterns that contradict constitution.md. If it needs to, create an ADR first (see `mjrp-adr` and `MJRP_templates/adr_template.md`).
