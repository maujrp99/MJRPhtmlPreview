...
name: mjrp-debug
description: >
  MJRP Debug Protocol for systematically tracking, finding, fixing, and documenting bugs with DEBUG_LOG.md.
  Trigger: debug, fix bug, there is a bug, something is broken, investigate issue, debugar, corrigir bug, erro.
---
...
1. **Grounding**: Load project context via `mjrp-onboarding` if not already loaded.
...
2. **Regression Check**: Run `mjrp-regression` for the affected area.
