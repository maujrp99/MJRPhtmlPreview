---
description: Architecture Review Protocol — Full system review using mjrp-system-architect skill before major milestones, after significant implementation, or when architectural drift is suspected.
---

# Architecture Review Protocol

**Purpose**: Trigger a structured architecture review using the `mjrp-system-architect` skill to catch layer violations, dependency drift, and technical debt before they compound. Distinct from the ADR protocol (which records a decision already made) — this protocol runs *before* decisions, to surface issues and inform them.
...
Follow the `mjrp-system-architect` skill's review sequence:
...
| New architectural decision needed | → `mjrp-architecture-decision-protocol.md` (ADR) |
| Docs out of sync with code | → `mjrp-documentation-audit-protocol.md` |
...
- **ADR (post-decision record)**: `mjrp-architecture-decision-protocol.md`
- **Doc-code sync**: `mjrp-documentation-audit-protocol.md`
- **Code quality scan**: `mjrp-code-quality-assessment-protocol.md`
- **SDD integration**: Run before `mjrp-sdd-specify-protocol.md` when starting a new milestone
